...
logging.basicConfig(filename='Log/HOME.log',
                    filemode='w',
                    encoding='utf-8',
    ...
    def __set__(self, instance, value):
        try:
            ...
            if not value.isalpha() or not value.istitle():
                raise ValueError(
                    "Фамилия должна содержать только буквы и начинаться c заглавной буквы.")
...
class Student:
    ...
    def average_scores_all_subjects(self):
        all_scores = [score for subject in self.subjects.values()
    ...
    def average_test_res_per_subj(self):
        ...
        for subject, test_result_data in self.subjects.items():
            ...
            average_test_results[subject] = round(
                sum(test_results) / len(test_results), 2)
        return average_test_results


if __name__ == '__main__':
    subjects_scores = {
        "Math": {"scores": [4, 5, 3], "test_results": [55, 78, 9]},
        "English": {"scores": [5, 4, 5], "test_results": [82, 47, 78]},
        "Science": {"scores": [3, 4, 4], "test_results": [7, 55, 58]}
    }
    student = Student("иванов", subjects_scores)
    print(f"Допустимые предметы: {set(student.subjects)}")
    print(
        f"Средний балл по всем предметам: {student.average_scores_all_subjects():.2f}")
    print(
        f"Средний балл по тестам по предмету: {student.average_test_res_per_subj()}")
